...
class CW_Sub_ens():
    def __init__(self, attackdir, savedir, model, model_few,
                 ...
                 sub_number=5,v_num=10,
                 c=1e-1,c_few=1e-1, kappa=0, steps=500, lr=0.08):
        self.model=model
        self.c = c
        self.v_num=v_num
        self.c_few = c_few
        ...
        for step in range(self.steps):
            ...
            if self.v_num!=0:
                ...
                for v in range(self.v_num):
                    adv_images.append(adv_images[0]+torch.Tensor(temp_w).uniform_(-0.01, 0.01).cuda())
                ...
                for n in range(n_):
                    ...
                    for v in range(self.v_num):
                        adv_images_fews.append(adv_images_fews[n] +
                                               torch.Tensor(temp_w).uniform_(-0.01, 0.01).cuda())
